...
# Build timestamp stamped into the image at build time (see Dockerfile, #101).
# Replaces the hand-bumped semver pill on the web UI: it's generated by the act
# of building, so it can't go stale or drift from the running image. Falls back
# to the process start time when running un-baked (local dev / tests).
import datetime as _datetime
_PROCESS_START = _datetime.datetime.now()
BUILD_STAMP_FILE = os.environ.get('EREADER_BUILD_STAMP_FILE', '/app/build-stamp.txt')
...
def get_build_time():
    ...
    at build time, so — unlike the semver pill — it can never be stale or
    mismatched against the running code; nothing to bump or re-stage."""
    return {'built': _read_build_stamp()}
